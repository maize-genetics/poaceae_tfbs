...
phyloK <- read.table("/workdir/coh22/andro_tfbs/output/modelData/phyloK_800Poaceae_astral.txt")
...
env.ogs <- read.table("/workdir/coh22/andro_tfbs/output/turnover_analysis/env_turnover/Yeaman2024RAO_OGID.txt") %>% select(c(at_id = V1, OG = V2))
gen.dist <- read.table("/workdir/coh22/andro_tfbs/output/angiosperm353_geneticDistance.txt", header = T)
taxa <- read.table("/workdir/coh22/andro_tfbs/output/angiosperm353_geneticDistance.txt", header = F)[1,]
...
taxa.to.filter.out <- read.csv("/workdir/coh22/andro_tfbs/lists/tabasco_under_3000.txt", header = F)
...
``` {r}
maize_gene_desc <- read.table("/workdir/coh22/andro_tfbs/output/B73_DE.out", sep = "\t", header = T) %>%
...
``` {r}
library(topGO)
mappingFile = read.table("/workdir/coh22/andro_tfbs/output/OGToZm_mapping_v2.txt") %>%
...
``` {r}
library(ggridges)
tf_ogs <- read.table("/workdir/coh22/andro_tfbs/output/turnover_analysis/tf_turnover/TF_OGs.txt") %>%
  dplyr::select(OG=V2)
...
syntenic_ogs <- read.table("/workdir/coh22/andro_tfbs/output/turnover_analysis/syntenic_only/ZmSb_syntenicOGs.txt") %>%
  dplyr::select(OG=V2)
...
# tf_ogs <- read.table("/workdir/coh22/andro_tfbs/output/TF_OGs.txt") %>%
#   dplyr::select(OG=V2)
# tf.og.ids <- tf_ogs$OG[tf_ogs$OG %in% names(filtered.mat.list)]
# tf.genes <- na.omit(Zm_vs_Sb_pct_conserved[tf.og.ids]) %>%
#   as.data.frame() %>%
...
Compare turnover of different motifs
``` {r}
...
taxa.to.filter.out <- read.table("/workdir/coh22/andro_tfbs/lists/fewerThan1000ogs.txt")
