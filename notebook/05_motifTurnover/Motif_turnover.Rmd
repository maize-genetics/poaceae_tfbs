...
ggplot(top_terms, aes(
  ...
  scale_size(range = c(5, 20)) +  # Only this line affects bubble size
  labs(
  ...
  theme_minimal() +  # Keeps your clean white background
  theme(
...
process_topGO <- function(ontology, allGenes, GODB, top_ogs, mappingFile) {
  ...
  significant_results <- GO_res_table_df %>%
    ...
    mutate(neglog10Fisher = -log10(as.numeric(Fisher.FDR))) %>%
    arrange(desc(neglog10Fisher))
...
# Merge results
merged_results_most_conserved <- bind_rows(significant_BP, significant_MF, significant_CC) %>% arrange(Fisher.FDR)
...
go_plot <- ggplot(top_terms, aes(
  ...
  scale_color_gradient(low = "red", high = "blue", limits = c(3, 7)) +
  scale_size(range = c(5, 20)) +  # Only this line affects bubble size
  labs(
    title = "GO enrichment for 25% most conserved genes",
    x = "-log10(FDR-corrected Fisher's exact test)", 
    y = NULL,
    size = "Significant",
    color = "-log10(Fisher)"
  ) +
...
motif_vs_cds_cons <- left_join(mean_turnover, mean.cds.conservation) %>%
  dplyr::rename(mean_motif_conservation = mean_pct_conserved) %>%
  dplyr::filter(mean_cds_conservation > 0.7)
...
ggplot(motif_vs_cds_cons, aes(x = mean_cds_conservation, y = mean_motif_conservation)) +
  geom_point(size = 1, alpha = 0.1) +                     # scatter plot points
  geom_smooth(method = "lm", se = T, color = "skyblue") + # optional regression line
  labs(
