...
# asymp_shared <- nls.models.by.gene %>%
#   dplyr::select(c(OG, asymp_shared = c_global)) %>%
#   left_join(OGkey) %>%
#   dplyr::filter(asymp_shared > 0.01)
# most_conserved <- asymp_shared[order(asymp_shared$asymp_shared, decreasing = T),][1:bin_length,]
# least_conserved <- asymp_shared[order(asymp_shared$asymp_shared, decreasing = F),][1:bin_length,]
...
#all_ogs <- unique(mappingFile[mappingFile[,2]%in%asymp_shared$OG,1])
...
go_plot <- ggplot(top_terms, aes(
  ...
  scale_color_gradient(low = "red", high = "blue", limits = c(1, 4)) +
...
#write.csv(merged_results_most_conserved[,c(2,3,4,5,6,8,10)], "/workdir/coh22/poaceae_tfbs/results/GO_low_conservation.csv", quote = F, row.names = F)
```
...
#tf.genes <- filter(asymp_shared, OG %in% tf.og.ids)
tf.genes$og.class <- "TF"
background.genes <- filter(mean_cons, !OG %in% tf.og.ids)
#background.genes <- filter(asymp_shared, !OG %in% tf.og.ids)
...
#test <- t.test(filter(merged.turnover, og.class == "non-TF")$asymp_shared, filter(merged.turnover, og.class == "TF")$asymp_shared, var.equal = TRUE)
...
#ks.test <- ks.test(tf.genes$asymp_shared, background.genes$asymp_shared)

# Ridgeline plot
means <- aggregate(mean_pct_conserved ~ og.class, data = merged.turnover, mean)
#means <- aggregate(asymp_shared ~ og.class, data = merged.turnover, mean)
# Plot density comparison with mean lines
#tf_nonTF <- ggplot(merged.turnover, aes(x = asymp_shared, fill = og.class)) +
...
motif_vs_cds_cons <- left_join(asymp_shared, mean.cds.conservation) %>%
