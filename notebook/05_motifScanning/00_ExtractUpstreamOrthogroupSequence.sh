# Pull out sequence 5kb upstream of orthologous genes
...
nJobs=$((threads / 40))
# NOTE:: Miniprot alignments were run on SciNet using 02_orthogroup/02_miniprotQuery.sh
...
# Then filter by ATG
parallel -j $threads "bash src/05_motifScanning/FilterGFF_byATG.sh {/.} {} output/miniProt_alignments/filtered_mRNA_stop_frameshift output/miniProt_alignments/filtered_mRNA_stop_frameshift_ATG 4" :::: lists/assembly_list.txt

echo "Extracting upstream sequence..."
# 5kb upstream (no length filter)
parallel -j $threads "bash src/05_motifScanning/ExtractUpstreamCoords.sh {/.} 5000 {//} output/miniProt_alignments/filtered_mRNA_stop_frameshift_ATG output/miniProt_alignments/filtered_mRNA_stop_frameshift_ATG_5kbUpstream F F" :::: lists/assembly_list.txt
