...
mkdir -p "${outDir}/intermed_alignments"

# unzip fasta file containing matK sequences from BOLD database
gunzip -c data/bold_matk.fas.gz > ${outDir}/bold_matk.fas

# Download Steprochaeta angustifolia matK seq from NCBI
efetch -db nuccore -id AF164382.1 -format fasta > ${outDir}/streptochaeta_angustifolia_matK.fasta
...
cat ${outDir}/intermed_alignments/*_matK.fa data/bold_matk.fas > ${outDir}/matK_combined_bold.fa
