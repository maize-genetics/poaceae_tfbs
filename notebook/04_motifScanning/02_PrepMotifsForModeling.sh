...
mkdir -p output/miniProt_alignments/filtered_mRNA_stop_frameshift_ATG_1000upstream_primaryAlignment 
...
parallel -j $threads "bash src/04_motifScanning/SummarizeMotifsByOrthogroup.sh {} \
   ...
   output/miniProt_alignments/filtered_mRNA_stop_frameshift_ATG_1000upstream_primaryAlignment \
   1000 \
   {//} \
...
cat $OG_dir/summarized_by_assembly/*.txt > $OG_dir/mergedMotifCounts_Poaceae800_1000upstream_primaryAlignment.txt
...
parallel -j "$threads" --no-notice "grep -E '{}($| )' $OG_dir/mergedMotifCounts_Poaceae800_1000upstream_primaryAlignment.txt > $OG_dir/summarized_by_OG/{}.txt" :::: lists/Helixer_OG_IDs_filtered.txt
