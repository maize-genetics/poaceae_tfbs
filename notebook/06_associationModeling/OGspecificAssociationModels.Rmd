...
```{r}
# Load covariates
setwd("/workdir/coh22/poaceae_tfbs/")
metadata <- read.csv('data/Poaceae_metadata_2025.04.07_CH.csv')
env <- read.table("output/modelData/envData_759Poaceae_20240821.txt") %>%
  mutate(assemblyID = rownames(.))
phyloK <- read.table("output/modelData/phyloK_728Poaceae_astral_20250407.txt")
tree <- read.tree("output/modelData/PoaceaeTree_angiosperm353_astral_filtered_20250407.nwk")
...
cat_models <- c(models_phyloK_envPC1.10_poaceae_1_5000, models_phyloK_envPC1.10_poaceae_5001_7000,
   ...
   lapply(unlist)
#orig.models <- lapply(models_phyloK_envPC1.10_poaceae_1_5000, unlist)
#load("/workdir/coh22/andro_tfbs/results/models/models_phyloK_envPC1.10_phyloPC1.5_poaceae_1_5000.RData")
#phyloPC.models <- lapply(models_phyloK_envPC1.10_poaceae, unlist)
...
#    lapply(unlist)
#model_output <- mclapply(phyloPC.models, function(x) {
...
#sparse_motifs <- do.call(rbind, lapply(names(mat.list)[1:5000], function(matrix_name) {
...
``` {r}
#orig.model.results <- results_df
#phyloPC.model.results <- results_df

orig <- dplyr::select(orig.model.results,c(OG,motif, ePC, orig_log10p = log10pval))
phyloPC <- dplyr::select(phyloPC.model.results,c(OG,motif, ePC, phyloPCs_log10p = log10pval))

results_comp <- left_join(phyloPC, orig)
plot(results_comp$orig_log10p, results_comp$phyloPCs_log10p)
p <- ggplot(results_comp, aes(x = orig_log10p, y = phyloPCs_log10p)) +
  geom_point(color = "darkgreen", size = 2, alpha = 0.2) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray40", linewidth = 1) +
  labs(
    x = "log10(p) for original models",
    y = "log10(p) for models with phyloPCs"
  )
p
```
...
# Save full
ggsave("/workdir/coh22/andro_tfbs/results/qqplot_envPC1_10_perOG.png", plot = PlotQQ(results_df), width = 10, height = 10, dpi = 300)
```
...
```{r}
library(ade4)
phyloPCaxes <- dudi.pca(phyloK, scannf = F, nf = 100)$li[1:100]
colnames(phyloPCaxes) <- paste0("phyloPC", 1:100)
```

```{r}
Run orthogroup-specific models with 5 phyloPCs
``` {r}
Run_MLMs_OrthoSpecific <- function(Kphylo, phyloPCs, EnvTraitMatrix, motifMatList) {
    # Check if the file exists and remove it before the loop
    filename <- "/workdir/coh22/poaceae_tfbs/output/output_log.txt"
    if (file.exists("/workdir/coh22/poaceae_tfbs/output/output_log.txt")) {
        file.remove(filename)
    ...
    ePC_names <- colnames(EnvTraitMatrix)
    phyloPC_names <- colnames(phyloPCs) #modified
  nOrtho <- length(motifMatList)
  cl <- makeCluster(1) # create cluster using 1 cores
  registerDoParallel(cl)
  results <- foreach(i = 1:nOrtho, .combine = 'c') %dopar% {
    library(dplyr)
    library(asreml)
    ...
    motifMat_filtered <- motifMatrix[common_rows,]
    phyloK_filtered <- Kphylo[common_rows,common_rows]
    phyloPCs_filtered <- phyloPCs[common_rows,] # modified for phyloPCs
    input_matrix <- as.data.frame(cbind(phyloPCs_filtered, EnvTraits_filtered, motifMat_filtered)) # modified for phyloPCs
    input_matrix$assemblyIDs <- factor(common_rows)
    ...
    motifModels <- list()
    for(j in 1:nMotifs){
    motifID <- colnames(input_matrix)[j+n_traits + n_phyloPCs] #modified
    # Check if more than 1 unique value
    if(length(unique(motifMat_filtered[,j])) > 1) {
        # Run association model for each motif
        formula_str <- paste(motifID, "~", 
                             paste(ePC_names, collapse =" + "),
                             "+", paste(phyloPC_names, collapse =" + "), # modified
                             collapse = " ") 
        formula_obj <- as.formula(formula_str)
...
#EnvTraitMatrix <- env[,286:287] # first 2 ePCs
rownames(EnvTraitMatrix) <- env$assemblyID
models_phyloK_envPC1.10_poaceae <- Run_MLMs_OrthoSpecific(phyloK, phyloPCaxes[,1:5], EnvTraitMatrix, mat.list[1:5000])
save(models_phyloK_envPC1.10_poaceae, file="/workdir/coh22/andro_tfbs/results/models/models_phyloK_envPC1.10_phyloPC1.5_poaceae_1_5000.RData")
```
