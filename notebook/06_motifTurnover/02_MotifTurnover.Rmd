...
# ogs found only at least 200 taxa
...
sum(is.na(nls.models.by.gene.list)) # 77 OGs failed to converge
...
stacked_curves <- ggplot(curves_long, aes(x = x, y = y, group = OG)) +
  ...
  labs(
    ...
    y = "% TF Motifs Retained or Regained"
  ) +
  ylim(0, 100) +
  theme_bw() +
  theme(
...
layered_plot <- stacked_curves +
  ...
  xlim(0, 0.25)
#ggsave(paste0(rootDir, "results/figs/motif_decay_500bp.jpeg"), plot = layered_plot, width = 12, height = 4, units = "in", dpi = 1500)
#ggsave(paste0(rootDir, "results/figs/motif_decay_500bp_sorghum.jpeg"), plot = layered_plot, width = 10, height = 4, units = "in", dpi = 1500)
#ggsave(paste0(rootDir, "results/figs/motif_decay_500bp_rice.jpeg"), plot = layered_plot, width = 10, height = 4, units = "in", dpi = 1500)
...
maize_gene_desc <- read.table(paste0(rootDir, "output/go_enrichment/B73_DE.out"), sep = "\t", header = TRUE, quote="", fill=TRUE) %>%
...
mean_cons <- apply(combined_results, 2, function(x) mean(x, na.rm = TRUE)) %>%
  ...
  mutate(OG = rownames(.)) %>%
  left_join(OGkey) %>%
  filter(OG %in% names(filtered_list))
...
indiv_gene <- pct_conserved_by_gene[["OG0026611"]]
...
nls_model_indiv <- fit_nls(prepped.ogs[["OG0026611"]])
...
top_terms <- merged_results[1:10,] # 10 most significant terms for ploting
...
go_plot <- ggplot(top_terms, aes(
  ...
  scale_color_gradient(low = "red", high = "blue", limits = c(1, 1.5)) +
  scale_size(range = c(5, 15)) + 
  labs(
    title = "Functional enrichment for genes in the bottom quartile of shared motif occupancy",
    x = "-log10(p)", 
    y = NULL,
    size = "Significant",
    color = "Enrichment"
...
#ggsave(paste0(rootDir, "results/figs/go_plot_highest_shared_occupancy.png"), go_plot, width = 8, height = 6, units = "in", dpi = 1500)
#ggsave(paste0(rootDir, "results/figs/go_plot_lowest_shared_occupancy.png"), go_plot, width = 8, height = 6, units = "in", dpi = 1500)
# Write all sig results to table
#write.csv(merged_results[,c(2,3,4,5,6,8,10)], "/workdir/coh22/poaceae_tfbs/results/GO_high_conservation.csv", quote = F, row.names = F)
#write.csv(merged_results[,c(2,3,4,5,6,8,10)], "/workdir/coh22/poaceae_tfbs/results/GO_low_conservation.csv", quote = F, row.names = F)
```

Compare turnover of TFs with non-TFs (maize TFs from Grassius: https://grassius.org/download_species_gene_list/Maize/v5) 
``` {r}
library(ggridges)
tf_ogs <- read.table(paste0(rootDir, "output/turnover_analysis/TF_OGs.txt")) %>%
  dplyr::select(OG=V2)
...
#ggsave(paste0(rootDir, "results/figs/tf_nonTF_shared_occupancy.png"), tf_nonTF, width = 8, height = 6, units = "in", dpi = 1500)
...
``` {r}
syntenic_ogs <- read.table(paste0(rootDir, "output/turnover_analysis/ZmSb_syntenicOGs.txt")) %>%
  dplyr::select(OG=V2)
...
motif_vs_cds_cons <- left_join(mean_cons, mean.cds.conservation) %>%
  ...
  #dplyr::rename(mean_motif_conservation = asymp_shared) %>%
...
# r-squared is just 0.02, though cds conservation is a highly significant predictor
...
ggplot(motif_vs_cds_cons, aes(x = mean_cds_conservation, y = mean_motif_conservation)) +
  ...
  labs(
    ...
    y = "Mean % shared motif occupancy vs Maize"
  ) +
...
# Shared occupancy rates by motif
``` {r}
...
reorient_data <- function(df_list, subset_names = NULL, cores = parallel::detectCores()) {
  ...
  # Name the list elements with motif names
...
stacked_curves <- ggplot(curves_long, aes(x = x, y = y, group = motif)) +
  ...
  labs(
    x = "Genetic Distance vs Maize",
    y = "% Motif instances with shared occupancy"
  ) +
  ylim(0, 100) +
  xlim(0,0.25) +
  theme_bw() +
...
# Initialize list to store shared occupancy data for each ChIP-motif pair
chip_prepped_motifs <- list()

# Loop over each motif–TF pair to calculate shared occupancy of that motif at ChIP-supported OGs
for(i in 1:nrow(mapping_df)) {
  ...
  # Calculate pct shared motif occupancy at the ChIP OGs
...
# Cuild a faceted scatter plot comparing shared motif occupancy at all OGs vs. chip OGs for each motif.
plot_data_list <- list()
...
p <- ggplot(plot_data, aes(x = all_conservation, y = chip_conservation)) +
  ...
  labs(
    x = "All OGs: % motifs with shared occupancy",
    y = "ChIP-supported OGs: % motifs with shared occupancy",
