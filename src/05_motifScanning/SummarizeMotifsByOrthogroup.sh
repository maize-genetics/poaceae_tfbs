...
bash src/05_motifScanning/ExtractUpstreamCoords.sh $assemblyID $distance_upstream $genomeFileDir $unfilt_interval_dir $temp_dir T T

# Filter out intervals containing more than 5% N
bash src/05_motifScanning/FilterRegionsContainingNs.sh $temp_dir/$assemblyID.bed $assemblyPath $filt_interval_dir/$assemblyID.bed 5 $assemblyID
