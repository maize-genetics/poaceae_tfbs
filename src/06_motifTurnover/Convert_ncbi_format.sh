...
# Check if motif_key is provided
if [ -z "$motif_key" ]; then
  # Set flag to indicate we shouldn't join with motif metadata
  metadata_join=false
else
  metadata_join=true
fi
...
if [ $metadata_join == false ]; then
  cat "$motifs_fixed_contig"
  rm "$sorted_genbank_key" "$motifs_fixed_contig"
  exit 0
  ...
  # Clean up temp files
  rm "$motifs_fixed_contig" "$sorted_genbank_key"
